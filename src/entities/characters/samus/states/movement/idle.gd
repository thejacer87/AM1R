...
func enter(previous_state_path: String, data := {}) -> void:
	...
	if samus.is_aiming and Input.is_action_pressed("diagonal_aim_" + str(samus.player_index)):
		animation += "aim_diag_" + ("down_" if aiming_down else "up_")
	samus.animation_player.play(animation + look_direction)
...
func physics_update(delta: float) -> void:
	...
	if Input.is_action_just_pressed("morph_" + str(samus.player_index)):
		...
		aiming_down = false
		samus.animation_player.play("stand_" + look_direction)
	elif Input.is_action_pressed("up_" + str(samus.player_index)):
		samus.animation_player.play("stand_aim_up_" + look_direction)
